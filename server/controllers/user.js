...
exports.findAll = (req, res) => {
    ...
    var condition =User.aggregate([{ $lookup:{from: "roles",localField: "role",foreignField: "_id",as: "Role_details"}}])
    User.find(condition)
    .populate("role")
      .then(data => {
        res.send(data);
...
exports.update = (req, res) => {
    ...
    User.findByIdAndUpdate(id,{name,employeeid,email,role,password,photo},{ useFindAndModify: false })
      .populate("role")
        .then(data => {
          if (!data) {
            res.status(404).send({
